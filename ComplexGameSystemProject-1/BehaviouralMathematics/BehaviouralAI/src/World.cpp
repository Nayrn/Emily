...
World::World()
{
	...
	for (int i = 0; i < 20; i++)
	{
		BaseNPC* pNPC = new GeneticsNPC(this);
		pNPC->SetColour(glm::vec4(i / 20.0f, i / 10.0f, 0, 1));
...
glm::vec3 World::getOtherLocation()
{
	//for (m_npcs = m_npcs.size; m_npcs.size() < m_genetics->)
	return m_restedLocation;
}
...
int World::CalcFitness()
{
	...
	pNPC->m_fLifeTime / 100;
	return pNPC->m_fLifeTime;
