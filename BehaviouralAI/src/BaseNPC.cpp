...
BaseNPC::BaseNPC(World* a_pWorld)
{
    ...
    m_uiFood = (rand() % 10 + 30);
    m_uiWater = (rand() % 10 + 30);
    m_uiRested = (rand() % 10 + 30);
    m_fLifeTime = 0.0f;
    m_uiNumberOfLogs = 0;
	...
	if (isIntroverted == true)
	{
		m_iBreedCost = 30.0f;
		std::cout << "Introverted" << std::endl;
	}
	else;
		std::cout << "Extroverted" << std::endl;
}
...
void BaseNPC::render()
{
    Gizmos::addAABBFilled(m_vPosition + glm::vec3(0, 2, 0), glm::vec3(0.5, 2, 0.5), m_renderColour);
}
...
void BaseNPC::breed(float a_fdeltaTime)
{
    if (travelTo(m_pWorld->getBreedLocation(), a_fdeltaTime))
    {
        if (m_pWorld->interactWithBreedLocation(this))
        {
			
			m_uiRested - m_iBreedCost;
			m_uiFood - m_iBreedCost;
...
void BaseNPC::updateLifeTime(float a_fdeltaTime)
{
    ...
    {
        m_bAlive = false;
        std::cout << "died of old age" << std::endl;
    }
}
