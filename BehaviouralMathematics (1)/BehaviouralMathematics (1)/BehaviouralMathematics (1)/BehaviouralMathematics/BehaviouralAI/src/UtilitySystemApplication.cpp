...
bool UtilitySystemApplication::startup() {
	...
	m_pUtNPC = new UtilityNPC(m_pWorld);
	
	m_pNPC->m_genetics = m_pNPC->m_genetics;

	for (int i = 0; i < m_pNPC->m_genetics->m_popSize; i++)
	{
		newRobot.push_back(BaseNPC(m_pWorld));
	}
	return true;
}
...
bool UtilitySystemApplication::update(float deltaTime) {
	...
	for (int i = 0; i < newRobot.size(); i++)
	{
		newRobot[i].update(deltaTime);
	}
	
	if (m_pNPC->reproduce(deltaTime))
	{
