...
{
	m_pWorld = a_pWorld;
	m_genetics = new Genetics(3, 1, 2, 4, 4);
	...
	m_fLastInteractReductionTime = 0.0f;
...
void BaseNPC::update(float a_fdeltaTime)
{
	...
	if (m_bAlive) 
	{
		...
		else if (m_uiWater < 30.0f)
		{
			selectAction1(a_fdeltaTime);
		}
		else if (m_uiRested < 25.0f)
		{
			selectAction2(a_fdeltaTime);
		}

		else if (m_uiRested > 30.0f && m_uiFood > 30.0f && m_uiWater > 30.0f)
		{
			selectAction3(a_fdeltaTime);
		}

		else if (m_uiRested > 25.0f && m_uiFood > 30.0f && m_uiWater > 30.0f && m_uiNumberOfLogs > 10.0f)
		{
			selectAction4(a_fdeltaTime);
		}
		else if (m_uiRested > 25.0f && m_uiFood > 30.0f && m_uiWater > 30.0f && m_uiLifeTime > 18.0f)
		{
			selectAction5(a_fdeltaTime);
		}

		if (m_bBreeding == true)
		{
			m_breedCost -= a_fdeltaTime;
			std::cout << m_breedCost << std::endl;
			if (reproduce(a_fdeltaTime));
			{
				m_genetics->m_popSize + 1;
			}
		}
		// can now successfully get logs to 10, now incorporate
		// successful reproduction		


		updateLifeTime(a_fdeltaTime);
...
//void BaseNPC::selectAction(float a_fdeltaTime)
//{
//}
//
//void BaseNPC::selectAction1(float a_fdeltaTime)
//{
//}
//
//void BaseNPC::selectAction2(float a_fdeltaTime)
//{
//}
//
//void BaseNPC::selectAction3(float a_fdeltaTime)
//{
//}
//
//void BaseNPC::selectAction4(float a_fdeltaTime)
//{
//}
//
//void BaseNPC::selectAction5(float a_fdeltaTime)
//{
//}
...
void BaseNPC::updateLifeTime(float a_fdeltaTime)
{
	 m_uiLifeTime += a_fdeltaTime;
}


bool BaseNPC::reproduce(float a_deltaTime)
{
	if (travelTo(m_pWorld->getOtherLocation(), a_deltaTime))
		if (m_uiLifeTime < 18.0f || m_uiLifeTime > 100.0f)
		{
			//std::cout << "Wrong age for breeding" << std::endl;
			return false;
		}
		else
		{
			if (m_pWorld->interactWithOther())
			{
...
void BaseNPC::checkAlive()
{
	...
	if (m_uiLifeTime == 100)
	{
		std::cout << "congrats, agent is old" << std::endl;
