...
public:
	...
	~BaseNPC();
	bool reproduce(float a_deltaTime);
...
protected:
	...
	virtual void selectAction(float a_fdeltaTime) { collectFood(a_fdeltaTime); };
	virtual void selectAction1(float a_fdeltaTime) { collectWater(a_fdeltaTime); };
	virtual void selectAction2(float a_fdeltaTime) { rest(a_fdeltaTime); };
	virtual void selectAction3(float a_fdeltaTime) { chopTree(a_fdeltaTime); };
	virtual void selectAction4(float a_fdeltaTime) { buildHouse(a_fdeltaTime); };
	virtual void selectAction5(float a_fdeltaTime) { reproduce(a_fdeltaTime); };
...
private:
	...
	float			m_fLastInteractReductionTime;
